...
cbuffer MaterialInfo : register(b10)
{
    ...
    float NormalMapIntensity_Slider = 0.0f;
...
{
    DefferedOutput result = (DefferedOutput)0;
    float3 WsPos = input.worldPosition.xyz;

    float2 ZyWsTexCoord = (WsPos.zy / TileSize) + float2(100.f, 3.f);
    float2 XzWsTexCoord = (WsPos.xz / TileSize) + 100.f;
    float2 XyWsTexCoord = (WsPos.xy / TileSize) + float2(100.f, 3.f);

    float2 RemappedWorldNormals = smoothstep(0.49f, 0.5f, abs(input.RotatedNormal.rg));
    
    float2 WsTexCoords = lerp(XyWsTexCoord, ZyWsTexCoord, RemappedWorldNormals.r);

    WsTexCoords = lerp(WsTexCoords, XzWsTexCoord, RemappedWorldNormals.g);
    
    float4 albedoSample = AlbedoTexture.Sample(WrapSampler, WsTexCoords).rgba;
    if (albedoSample.a <= 0.5f)
    {
        return result;
    }

    float3 normalMap = NormalMapTexture.Sample(WrapSampler, WsTexCoords).agb;
    float4 material = MaterialInfoTexture.Sample(WrapSampler, WsTexCoords).rgba;
