...
namespace ImGui
{
	...
	bool ImGradientHDRState::AddColorMarker(const ColorMarker& aColorMarker)
	{
		return AddColorMarker(aColorMarker.Position, aColorMarker.Color, aColorMarker.Intensity);
	}
	...
	std::array<float, 4> ImGradientHDRState::GetColorAndIntensity(float x) const
	{
		...
		const auto it = std::lower_bound(Colors.begin(), Colors.begin() + ColorCount, key, [](const ColorMarker& a, const ColorMarker& b)
										 { return a.Position < b.Position; });
	...
	float ImGradientHDRState::GetAlpha(float x) const
	{
		...
		const auto it = std::lower_bound(Alphas.begin(), Alphas.begin() + AlphaCount, key, [](const AlphaMarker& a, const AlphaMarker& b)
										 { return a.Position < b.Position; });
	...
	bool ImGradientHDR(const char* gradientID, ImGradientHDRState& state, ImGradientHDRTemporaryState& temporaryState, bool isMarkerShown)
	{
		bool changed = false;
		...
		constexpr auto barHeight = 20;
		constexpr auto markerWidth = 15;
		constexpr auto markerHeight = 15;
		...
		for (int y = 0; y * gridSize < barHeight; y += 1)
		{
			...
			{
				const int wgrid = std::min(gridSize, static_cast<int>(width) - x * gridSize);
				const int hgrid = std::min(gridSize, barHeight - y * gridSize);
...

		{
			...
			const auto result = std::unique(xkeys.begin(), xkeys.end());
