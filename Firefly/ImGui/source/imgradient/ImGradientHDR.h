...
namespace ImGui
{
	...
	struct ImGradientHDRState
	{
		...
		bool AddColorMarker(const ColorMarker& aColorMarker);
	...
	inline void ImGradientHDRState::Clear()
	{
		memset(this, 0, sizeof(ImGradientHDRState));
	}
	...
	bool ImGradientHDR(const char* gradientID, ImGradientHDRState& state, ImGradientHDRTemporaryState& temporaryState, bool isMarkerShown = true);
}