...
namespace ImGui
{
	...
	//BENNE FUNCTIONS
	NOTIFY_INLINE void NotifyInfo(const char* format, ...)
	{
		ImGuiToast toast(ImGuiToastType_Info);
		NOTIFY_FORMAT(toast.set_content, format);
		
		InsertNotification(toast);
	}

	NOTIFY_INLINE void NotifySuccess(const char* format, ...)
	{
	...
	NOTIFY_INLINE void NotifyWarning(const char* format, ...)
	{
	...
	NOTIFY_INLINE void NotifyError(const char* format, ...)
	{
	...
	NOTIFY_INLINE void NotifyInfoLocal(const char* format, ...)
	{
		ImGuiToast toast(ImGuiToastType_Info);
		toast.currentWindow = true;
		toast.windowPos = GetWindowPos();
		toast.windowSize = GetWindowSize();
		NOTIFY_FORMAT(toast.set_content, format);

		InsertNotification(toast);
	}

	NOTIFY_INLINE void NotifySuccessLocal(const char* format, ...)
	{
		ImGuiToast toast(ImGuiToastType_Success);
		toast.currentWindow = true;
		toast.windowPos = GetWindowPos();
	...
	NOTIFY_INLINE void NotifyWarningLocal(const char* format, ...)
	{
		ImGuiToast toast(ImGuiToastType_Warning);
		toast.currentWindow = true;
		toast.windowPos = GetWindowPos();
	...
	NOTIFY_INLINE void NotifyErrorLocal(const char* format, ...)
	{
		ImGuiToast toast(ImGuiToastType_Error);
		toast.currentWindow = true;
		toast.windowPos = GetWindowPos();
	...
	NOTIFY_INLINE void RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			//BENNE FIX
			if (current_toast->currentWindow)
			{
				SetNextWindowPos(ImVec2(
					current_toast->windowPos.x + current_toast->windowSize.x - NOTIFY_PADDING_X,
					current_toast->windowPos.y + current_toast->windowSize.y - NOTIFY_PADDING_Y - localHeight), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			}
			else
			{
				SetNextWindowPos(ImVec2(vp_pos.x + vp_size.x - NOTIFY_PADDING_X, vp_pos.y + vp_size.y - NOTIFY_PADDING_Y - globalHeight), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			}
			//OLD CODE
			//SetNextWindowPos(ImVec2(vp_size.x - NOTIFY_PADDING_X, vp_size.y - NOTIFY_PADDING_Y - globalHeight), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			//
			...
			// Save globalHeight for next toasts
			if (!current_toast->currentWindow)
				globalHeight += GetWindowHeight() + NOTIFY_PADDING_MESSAGE_Y;
			else
				localHeight += GetWindowHeight() + NOTIFY_PADDING_MESSAGE_Y;
